base_path = 'C:\';
figname = 'op_pess_mice_lick_sucrose_microstructure_plot';
...
% lick microstructure options

opts = struct();
opts.lickRateVar = 'Licking_10s_baseline'; % your per-trial vector of lick rate (Hz)
opts.binSize = 0.1;                       % 100 ms
opts.tStart  = -10;                       % adjust to your timebase
opts.win     = [6.3 11.2];

opts.lickOnThreshold = 0;                 % bin is "licking" if rate > 0
opts.maxSilentGapWithinBurst_sec = 0.1;   % bridge gaps up to 200 ms (2 bins)
opts.minBurstBins = 2;                    % require >=2 ON bins to be a burst

opts.includePartialFor_nBursts  = true;
opts.includePartialFor_duration = true;
opts.includePartialFor_freq     = true;

opts.partialHandling_duration = 'clip';
opts.partialHandling_freq     = 'clip';
...
lickMT = licking_microstructure_by_trial_binned(data1_t, opts);
data1_t = [data1_t lickMT.trialTable];
data1_t.LMEcat = categorical(repmat(1,[size(data1_t,1) 1]));
data1 = nanmean(data1_t.Licking_10s_baseline(:,data_int),1);
data1_bar = data1_t.burstDurMean_sec;
[data1_m_temp, mouseIDs] = average_signal_per_mouse(data1_t, 'burstDurMean_sec');
data1_m = data1_m_temp;
data1_m_bar = data1_m_temp;
...
lickMT = licking_microstructure_by_trial_binned(data2_t, opts);
data2_t = [data2_t lickMT.trialTable];
data2_t.LMEcat = categorical(repmat(2,[size(data2_t,1) 1]));
data2 = nanmean(data2_t.Licking_10s_baseline(:,data_int),1);
data2_bar = data2_t.burstDurMean_sec;
[data2_m_temp, mouseIDs] = average_signal_per_mouse(data2_t, 'burstDurMean_sec');
data2_m = data2_m_temp;
data2_m_bar = data2_m_temp;
...
lickMT = licking_microstructure_by_trial_binned(data3_t, opts);
data3_t = [data3_t lickMT.trialTable];
data3_t.LMEcat = categorical(repmat(3,[size(data3_t,1) 1]));
data3 = nanmean(data3_t.Licking_10s_baseline(:,data_int),1);
data3_bar = data3_t.burstDurMean_sec;
[data3_m_temp, mouseIDs] = average_signal_per_mouse(data3_t, 'burstDurMean_sec');
data3_m = data3_m_temp;
data3_m_bar = data3_m_temp;
...
% data4_t = data4_t(data4_t.TrialTypeGrp == '6or9',:);
lickMT = licking_microstructure_by_trial_binned(data4_t, opts);
data4_t = [data4_t lickMT.trialTable];
data4_t.LMEcat = categorical(repmat(4,[size(data4_t,1) 1]));
data4 = nanmean(data4_t.Licking_10s_baseline(:,data_int),1);
data4_bar = data4_t.burstDurMean_sec;
[data4_m_temp, mouseIDs] = average_signal_per_mouse(data4_t, 'burstDurMean_sec');
data4_m = data4_m_temp;
data4_m_bar = data4_m_temp;
...
lickMT = licking_microstructure_by_trial_binned(data5_t, opts);
data5_t = [data5_t lickMT.trialTable];
data5_t.LMEcat = categorical(repmat(5,[size(data5_t,1) 1]));
data5 = nanmean(data5_t.Licking_10s_baseline(:,data_int),1);
data5_bar = data5_t.burstDurMean_sec;
[data5_m_temp, mouseIDs] = average_signal_per_mouse(data5_t, 'burstDurMean_sec');
data5_m = data5_m_temp;
data5_m_bar = data5_m_temp;
...
% data6_t = data6_t(data6_t.TrialTypeGrp == '6or9',:);
lickMT = licking_microstructure_by_trial_binned(data6_t, opts);
data6_t = [data6_t lickMT.trialTable];
data6_t.LMEcat = categorical(repmat(6,[size(data6_t,1) 1]));
data6 = nanmean(data6_t.Licking_10s_baseline(:,data_int),1);
data6_bar = data6_t.burstDurMean_sec;
[data6_m_temp, mouseIDs] = average_signal_per_mouse(data6_t, 'burstDurMean_sec');
data6_m = data6_m_temp;
data6_m_bar = data6_m_temp;
...
[data7_t] = filter_BigT_121925(in_t, exclude_raw_n,  TrainingT,TrialT,lick_discrim_thresh,lick_discrim_cat,neural_discrim_thresh,neural_discrim_cat,lick_discrim_thresh_posodor,lick_discrim_cat_posodor,neural_discrim_thresh_posodor,neural_discrim_cat_posodor,lick_discrim_thresh_negodor,lick_discrim_cat_negodor,neural_discrim_thresh_negodor,neural_discrim_cat_negodor,op_pess_mice,skew_thresh,op_pess_trials,licking_filter,lick_filter_int,select_mice_list,exclude_mice_list);
lickMT = licking_microstructure_by_trial_binned(data7_t, opts);
data7_t = [data7_t lickMT.trialTable];
data7_t.LMEcat = categorical(repmat(7,[size(data7_t,1) 1]));
data7 = nanmean(data7_t.Licking_10s_baseline(:,data_int),1);
data7_bar = data7_t.burstDurMean_sec;
[data7_m_temp, mouseIDs] = average_signal_per_mouse(data7_t, 'burstDurMean_sec');
data7_m = data7_m_temp;
data7_m_bar = data7_m_temp;
...
% data8_t = data8_t(data8_t.TrialTypeGrp == '6or9',:);
lickMT = licking_microstructure_by_trial_binned(data8_t, opts);
data8_t = [data8_t lickMT.trialTable];
data8_t.LMEcat = categorical(repmat(8,[size(data8_t,1) 1]));
data8 = nanmean(data8_t.Licking_10s_baseline(:,data_int),1);
data8_bar = data8_t.burstDurMean_sec;
[data8_m_temp, mouseIDs] = average_signal_per_mouse(data8_t, 'burstDurMean_sec');
data8_m = data8_m_temp;
data8_m_bar = data8_m_temp;
